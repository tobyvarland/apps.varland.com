module ApplicationHelper
  ...
  def primary_nav_link(icon, text, url, options = {})
    ...
    link_to_unless_current(html, url, class: "nav-link") do content_tag(:div, html, class: "navbar-text px-md-2") end
  end

  def flash_message(msg, type)
    message = content_tag(:div, msg.html_safe, class: "message")
    icon = case type
           when :alert then content_tag(:div, fa("exclamation", text_class: "text-danger"), class: ["icon", "rounded-start"])
           else content_tag(:div, fa("info", text_class: "text-info"), class: ["icon", "rounded-start"])
           end
