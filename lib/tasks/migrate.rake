...
def migrate(from, to)
  ...
  puts "#{ActiveStorage::Blob.where(service_name: "local").where("id > ?", 180).count} Blobs to go..."

  ActiveStorage::Blob.where(service_name: "local").where("id > ?", 180).each do |blob|
